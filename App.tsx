...
// ====== Lead capture config (optional) ======
const LEAD_WEBHOOK = ""; // e.g. "https://your-domain.com/api/lead"
const TG_BOT_TOKEN = ""; // e.g. "123456:ABCDEF..." (leave empty to use runtime globals)
const TG_CHAT_ID = "";   // e.g. 123456789 or "@your_channel" (leave empty to use runtime globals)

declare global {
  interface Window { __TG_TOKEN__?: string; __TG_CHAT__?: string; }
}
  ...
  const strip = (key: string, x: number) => (
    ...
    >
      {items.map((t: string, i: number) => (
        <span key={`${key}-${i}`} className="opacity-80">
          {t}
        </span>
      ))}
    ...
    <header className="sticky top-0 z-40 border-b border-white/5 bg-zinc-950/70 backdrop-blur">
      <nav id="main-nav" className="mx-auto max-w-6xl px-4 sm:px-6 lg:px-8 py-3 flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          <button onClick={onQuiz} className="hidden sm:inline-flex items-center gap-2 rounded-lg bg-white px-3 py-2 text-sm font-medium text-zinc-900">
      ...
      {open && (
        <div className="md:hidden border-t border-white/10 bg-zinc-950/95 backdrop-blur">
          <div className="mx-auto max-w-6xl px-4 py-3 grid gap-2 text-base">
            ...
            <button onClick={() => { toggle(); onQuiz(); }} className="mt-2 inline-flex items-center justify-center gap-2 rounded-lg bg-white px-4 py-3 font-semibold text-zinc-900">Написать</button>
          </div>
        </div>
      )}
    </header>
      ...
      <motion.div variants={container} initial="hidden" animate="show" className="grid grid-cols-1 lg:grid-cols-12 gap-8 sm:gap-10 items-center">
        <motion.div variants={item} className="lg:col-span-7">
          ...
          <div className="mt-8 flex flex-col sm:flex-row sm:flex-wrap gap-3">
            <button onClick={onQuiz} className="inline-flex items-center justify-center rounded-xl bg-zinc-900 px-5 py-3 text-base sm:text-sm font-semibold text-white w-full sm:w-auto">Запустить траф</button>
      ...
      <ul className="mt-6 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 text-sm">
        {groups.map((g, i) => (
          <li key={i} className="rounded-2xl border border-white/10 p-4 flex flex-col justify-between">
            ...
            <button onClick={onQuiz} className="mt-4 inline-flex items-center justify-center rounded-xl bg-white text-black px-4 py-2 text-sm font-medium w-full">Запустить траф</button>
          </li>
        ))}
      </ul>
    </Section>
      ...
      <ul className="mt-6 grid grid-cols-1 md:grid-cols-3 gap-3">
        {plans.map((p, i) => (
          <li key={i} className={`rounded-2xl border p-5 ${p.highlight ? "border-zinc-300" : "border-zinc-200"}`}>
            {p.highlight && (
              <span className="mb-3 inline-block rounded-full border border-zinc-300 px-2 py-0.5 text-xs">Популярный</span>
            )}
            ...
            <ul className="mt-4 space-y-1 text-sm text-zinc-600">
              {p.features.map((f, idx) => (
                <li key={idx} className="flex items-center gap-2"><Check className="h-4 w-4" /> {f}</li>
              ))}
            </ul>
            <button onClick={onQuiz} className="mt-5 inline-flex items-center justify-center rounded-xl bg-zinc-900 px-4 py-3 text-sm font-medium text-white w-full">Берем</button>
          </li>
        ))}
      </ul>
    </Section>
    ...
    <div className="fixed inset-0 z-[100] flex items-end sm:items-center justify-center p-0 sm:p-6">
      ...
      <div className="relative z-[101] w-full sm:max-w-lg rounded-t-2xl sm:rounded-2xl bg-white text-zinc-900 shadow-2xl">
        <div className="flex items-center justify-between px-5 py-4 border-b border-zinc-200">
          ...
          <button onClick={onClose} className="inline-flex h-9 w-9 items-center justify-center rounded-md border border-zinc-200">x</button>
        </div>
        ...
        <div>
          ...
          {step === questions.length && (
            <div className="px-5 py-5 space-y-4">
              ...
              <p className="text-sm text-zinc-600">Если Telegram не открылся — нажмите кнопку ниже. Чтобы передать ответы, выделите текст и нажмите <b>Ctrl</b>/<b>Cmd</b> + C, или долгий тап на телефоне → <span className="font-medium">Копировать</span>.</p>
...
export function TraffAgentLanding() {
  ...
  useEffect(() => {
    const defs = { Header, Hero, Metrics, Services, Inside, Cases, Pricing, FAQ, Footer };
  ...
  return (
    <div className="min-h-screen bg-zinc-950 text-zinc-200">
      ...
      <div className="fixed bottom-3 inset-x-3 sm:hidden z-30">
        <button onClick={() => setQuizOpen(true)} className="flex items-center justify-center rounded-xl bg-white text-zinc-900 px-4 py-3 text-base font-semibold shadow-lg shadow-black/30 w-full">Берем - обсудить проект</button>
      </div>
