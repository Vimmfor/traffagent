      ...
      <motion.div variants={container} initial="hidden" animate="show" className="relative grid grid-cols-1 lg:grid-cols-12 gap-8 sm:gap-10 items-center">
        <motion.div variants={item} className="lg:col-span-7">
          ...
          <h1 className="mt-2 text-3xl sm:text-4xl md:text-6xl font-extrabold leading-snug">
            TraffAgent <span className="gradient-text">— трафик за гранью разума</span>
          </h1>
...
function Services({ onQuiz }: { onQuiz: () => void }) {
  ...
  return (
    <Section id="services" className="py-12" bg="bg-zinc-950 text-zinc-100">
      ...
      <H2>Услуги</H2>
      <ul className="mt-6 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 text-sm">
        {groups.map((g, i) => (
          <li key={i} className="card rounded-2xl border border-white/10 p-4 bg-gradient-to-b from-white/5 to-transparent">
            ...
            <button type="button" onClick={onQuiz} className="mt-4 inline-flex items-center justify-center rounded-xl bg-white text-black px-4 py-2 text-sm font-medium w-full hover:bg-zinc-100 transition-colors">Запустить траф</button>
          </li>
        ))}
      </ul>
    </Section>
...
function Pricing({ onQuiz }: { onQuiz: () => void }) {
  ...
  return (
    <Section id="pricing" className="py-12" bg="bg-white text-zinc-900">
      ...
      <ul className="mt-6 grid grid-cols-1 md:grid-cols-3 gap-4">
        {plans.map((p, i) => (
          <li key={i} className={`card rounded-2xl border p-5 bg-white/80 backdrop-blur ${p.highlight ? "border-zinc-300" : "border-zinc-200"}`}>
            ...
            <button type="button" onClick={onQuiz} className="mt-5 inline-flex items-center justify-center rounded-xl bg-gradient-to-r from-indigo-500 via-purple-500 to-fuchsia-500 px-4 py-3 text-sm font-medium text-white w-full shadow-lg hover:shadow-xl transition-shadow">
              Берем
            </button>
          </li>
        ))}
