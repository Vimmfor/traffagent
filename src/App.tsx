...
function KPMobileHero({ onQuiz }: { onQuiz: () => void }) {
  // Плотнее верх: уменьшил pt, убрал лишние промежутки, оставил edge-to-edge
  return (
    <Section id="home" className="min-h-[82vh] flex flex-col justify-center pt-4 pb-8 sm:pt-10 sm:pb-12" bg="bg-white text-black">
  ...
  return (
    <>
      ...
      <div className="hidden sm:block">
        <Section id="home" className="pt-12 pb-12 sm:pt-14 sm:pb-14" bg="bg-white text-zinc-900">
          <motion.div variants={container} initial="hidden" animate="show" className="grid grid-cols-1 lg:grid-cols-12 gap-8 sm:gap-10 items-center">
            <motion.div variants={item} className="lg:col-span-7">
              <div className="bg-red-600 text-white px-3 py-2 flex items-center justify-between rounded-md mb-5">
                <div className="text-[11px] font-extrabold uppercase tracking-wider">Срочно</div>
                <div className="text-[11px] font-semibold">Объявление</div>
              </div>
              ...
              <div className="mt-7 flex flex-col sm:flex-row sm:flex-wrap gap-3">
                ...
                <button
                  onClick={() => { fbqTrack('Lead', { place: 'hero_tg' }); onQuiz(); }}
...
function QuizModal({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  const choose = (opt: string) => {
    ...
    else setStep(step + 1) // откроется финальный экран
  }
